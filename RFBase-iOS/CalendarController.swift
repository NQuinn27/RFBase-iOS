...
//  RFBase-iOS
//
//  Created by niall quinn on 23/04/2017.
...
class CalendarController: UINavigationController {
  fileprivate let url = URL(string: "http://192.168.1.5:3000/api/v1/calendar")!
  ...
  fileprivate lazy var webViewConfiguration: WKWebViewConfiguration = {
    ...
    configuration.applicationNameForUserAgent = "Calendar"
...
extension CalendarController: SessionDelegate {
  ...
  func session(_ session: Session, didFailRequestForVisitable visitable: Visitable, withError error: NSError) {
    NSLog("ERROR: %@", error)
    guard let demoViewController = visitable as? CalendarViewController, let errorCode = ErrorCode(rawValue: error.code) else { return }
