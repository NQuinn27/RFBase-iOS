...
class PostsController: UINavigationController {
  fileprivate let url = URL(string: "http://192.168.1.5:3000/api/v1/post")!
  ...
  fileprivate lazy var webViewConfiguration: WKWebViewConfiguration = {
    ...
    configuration.applicationNameForUserAgent = "Posts"
  ...
  fileprivate func presentVisitableForSession(_ session: Session, url: URL, action: Action = .Advance) {
    let visitable = PostsViewController(url: url)
...
extension PostsController: SessionDelegate {
  ...
  func session(_ session: Session, didFailRequestForVisitable visitable: Visitable, withError error: NSError) {
    NSLog("ERROR: %@", error)
    guard let demoViewController = visitable as? PostsViewController, let errorCode = ErrorCode(rawValue: error.code) else { return }
...
extension PostsController: WKScriptMessageHandler {
